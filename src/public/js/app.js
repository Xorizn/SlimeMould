...
async function init() {
  try {
    ...
    const handleSearch = (e) => {
      ...
      const filtered = addressData.filter(
        (item) =>
    ...
    document.getElementById("status-bar").textContent =
      `${addressData.length} nodes mapped | Hover to inspect | Click to focus`;
  } catch (err) {
...
function handleClick(e) {
  if (hoveredId) selectAddress(hoveredId, true);
}
...
function updateHovered(mapX, mapY) {
  ...
  addressData.forEach((item) => {
    const dist = Math.hypot(item.koordinat.x - mapX, item.koordinat.y - mapY);
    if (dist < minDist) {
      minDist = dist;
      closest = item.id;
    }
...
function selectAddress(id, scroll = false) {
  ...
  document.querySelectorAll(".address-item").forEach((el) => {
    ...
    el.classList.toggle(
      "selected",
      elId === id ||
        (pointA && elId === pointA.id) ||
        (pointB && elId === pointB.id),
    );
    if (scroll && elId === id) {
...
function drawNode(item, time) {
  ...
  if (isHovered && !isSelected && !isA && !isB) {
    ctx.fillStyle = "white";
    ctx.font = `${Math.max(10, 10 * scale)}px monospace`;
...
function updateSearchBranches() {
  ...
  searchBranches.forEach((branch) => {
    ...
    if (possibleNext.length > 0) {
      ...
      if (nextNode.id === pointB.id) branch.finished = true;
    } else {
